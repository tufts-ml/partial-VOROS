...
def area(polygon_points):
    ...
    """
    polygon_points = jnp.atleast_2d(polygon_points)
    if polygon_points.shape[0] == 0 or polygon_points.shape[1] < 2:
        return 0.0
    ...
    is_unique_edge = jnp.any(diffs > 1e-12, axis=1) & (~is_nan)
    num_valid = jnp.sum(is_unique_edge)
    
    safe_num_valid = jnp.where(num_valid == 0, 1.0, num_valid)
    valid_vertices = jnp.where(is_unique_edge[:, None], clean_poly, 0.0)
    
    cx = jnp.sum(valid_vertices[:, 0]) / safe_num_valid
    cy = jnp.sum(valid_vertices[:, 1]) / safe_num_valid
    ...
    # Roll ordered_poly by -1 to get next vertices
    # For valid vertices 0..k-1, vertex i's next vertex is (i+1)%k
    idx = jnp.arange(polygon_points.shape[0])
    next_idx = (idx + 1) % jnp.maximum(num_valid, 1)
    
    x = ordered_poly[:, 0]
    y = ordered_poly[:, 1]
    
    cross_products = x * y[next_idx] - y * x[next_idx]
    valid_mask = idx < num_valid
    signed_area_sum = jnp.sum(jnp.where(valid_mask, cross_products, 0.0))
    
    final_area = jnp.where(num_valid < 3, 0.0, 0.5 * jnp.abs(signed_area_sum))
...
def _clip_polygon_with_halfplane(poly, a, b, c):
    ...
    Normalizes (a, b, c) locally to guarantee float64 numerical stability.
    """
    # NORMALIZE LOCALLY TO PREVENT PRECISION LOSS AT LARGE SLOPES (e.g., m = 1e6)
    norm = jnp.sqrt(a**2 + b**2)
    safe_norm = jnp.where(norm < 1e-15, 1.0, norm)
    a = a / safe_norm
    b = b / safe_norm
    c = c / safe_norm
    ...
    return jnp.where(is_still_valid, clipped_poly, jnp.nan)


def _sort_and_clean_polygon(poly):
    """
    Cleans and sorts a padded JAX polygon counter-clockwise.
    Preserves exact vertex geometry matching geometry.py.
    """
    ...
    # Stable secondary sorting key using point index
    idx_arr = jnp.arange(poly.shape[0], dtype=jnp.float64)
    sorted_angles = jnp.where(is_unique_edge, angles, 10.0 + idx_arr * 1e-5)
    
    sorted_idx = jnp.argsort(sorted_angles)
    ordered_poly = clean_poly[sorted_idx]
    
    last_valid_idx = jnp.clip(num_valid - 1, 0, poly.shape[0] - 1)
    last_valid_val = ordered_poly[last_valid_idx]
...
def _intersect_halfplanes(halfplanes, bbox=((0, 0), (1, 1))):
    ...
    return _sort_and_clean_polygon(poly)
...
def compute_total_region_polygon(P, N, α, κ):
    ...
    h0 = jnp.array([m_p, -1.0, 0.0])
    capacity_plane = jnp.array([N, P, κ])
    
    # FIX: Use 1e9 so non-binding capacity doesn't cause zero-denom clipping artifacts
    dummy_plane = jnp.array([0.0, 0.0, 1e9])
    h_cap = jnp.where(κ < (P + N), capacity_plane, dummy_plane)
    
    h_x_min = jnp.array([-1.0, 0.0, 0.0])
    h_y_max = jnp.array([0.0, 1.0, 1.0])
    h_x_max = jnp.array([1.0, 0.0, 1.0])
    h_y_min = jnp.array([0.0, -1.0, 0.0])
    
    halfplanes = jnp.stack([h0, h_cap, h_x_min, h_y_max, h_x_max, h_y_min], axis=0)
    return _intersect_halfplanes(halfplanes, bbox=((0.0, 0.0), (1.0, 1.0)))
...
def _iso_performance_line(h, k, t):
    ...
    m_clean = jnp.where(jnp.abs(m) < 1e-12, 0.0, m)
    
    a_slope = jnp.where(m_clean >= 0, -m_clean, m_clean)
    b_slope = jnp.where(m_clean >= 0, 1.0, -1.0)
    c_slope = jnp.where(m_clean >= 0, b0, -b0)
    ...
    # 2. Vertical line override (x >= x1 => a=-1, b=0, c=-x1)
    a_vert = jnp.where(is_vertical, -1.0, a_base)
    b_vert = jnp.where(is_vertical, 0.0, b_base)
    c_vert = jnp.where(is_vertical, -x1, c_base)
    
    # 3. t = 0 HIGHEST PRECEDENCE (matches _geometry.py guard order)
    a = jnp.where(is_t_zero, 0.0, a_vert)
    b = jnp.where(is_t_zero, 1.0, b_vert)
    c = jnp.where(is_t_zero, k, c_vert)
    
    return a, b, c

# ---- Reduced area ----

def reduced_area(h, k, κ, α, P, N, fp_cost_ratio, 
                 total_poly_area=None, total_poly=None,
                 return_percent=True, return_details=False, return_total_area=False):
    r = fp_cost_ratio
    t = r * N / (r * N + P)
    
    # # Compute base feasible region ONLY if not passed in
    if total_poly_area is None or total_poly is None:
...
@jax.jit(static_argnames=['n_points', 'return_best_thresholds', 'do_fast_threshold_sel_via_cost'])
def max_area_per_t(fprs, tprs, κ, α, P, N, min_fp_cost_ratio, max_fp_cost_ratio,
    ...
    # 1. COMPUTE TOTAL REGION POLYGON EXACTLY ONCE
    total_poly_area, total_poly = total_region_area(P, N, α, κ)
    ...
    def process_single_ratio(fp_ratio, t_val):
        def fast_selection_path():
            ...
            best_area = reduced_area(
                fprs[imax], tprs[imax], κ, α, P, N, fp_ratio,
                total_poly_area=total_poly_area, total_poly=total_poly
            )
        ...
        def exhaustive_search_path():
            vmap_reduced_area = jax.vmap(
                lambda f, t: reduced_area(
                    f, t, κ, α, P, N, fp_ratio,
                    total_poly_area=total_poly_area, total_poly=total_poly
                )
            )
            all_areas = vmap_reduced_area(fprs, tprs)
            
            # Deterministic argmax tie-breaker
            idx_tie_breaker = jnp.arange(fprs.shape[0], dtype=jnp.float64) * 1e-15
            imax = jnp.where(fprs.shape[0] > 0, jnp.argmax(all_areas - idx_tie_breaker), 0)
    ...
    # 2. Vectorize single ratio across pre-computed polygon
...
def voros_jax(
    ...
    do_fast_threshold_sel_via_cost: bool = False):
    ...
    dx = (max_fp_cost_ratio - min_fp_cost_ratio) / jnp.maximum(n_points - 1, 1)
    integral_val = 0.5 * dx * (max_points[0] + max_points[-1] + 2.0 * jnp.sum(max_points[1:-1]))
    
    vor_integrated = integral_val / jnp.maximum(r_range, 1e-15)

    is_valid_range = (n_points > 1) & (r_range > 0)
    vor = jnp.where(is_valid_range, vor_integrated, max_points[0])

    if return_best_thresholds:
        return vor, jnp.asarray(ts, dtype=jnp.float64), jnp.asarray(best_thresholds, dtype=jnp.float64)
...
def keep_model(fpr, tpr, target_prec, target_cap, count_N, count_P):
    ...
    lower_bound = (target_prec * count_N * fpr) / ((1.0 - target_prec) * count_P)
...
#     cross_products = x * y_next - y * x_next
    
#     # Only sum cross products for valid vertices (< num_valid)
#     valid_mask = idx < num_valid
#     signed_area_sum = jnp.sum(jnp.where(valid_mask, cross_products, 0.0))
...
#     # Safe count for denominator
#     safe_num_valid = jnp.where(num_valid == 0, 1.0, num_valid)
    
#     # 2. Compute centroid strictly over valid points
#     valid_vertices = jnp.where(is_unique_edge[:, None], clean_poly, 0.0)
#     cx = jnp.sum(valid_vertices[:, 0]) / safe_num_valid
#     cy = jnp.sum(valid_vertices[:, 1]) / safe_num_valid
    
#     # 3. Calculate polar angles
#     angles = jnp.arctan2(clean_poly[:, 1] - cy, clean_poly[:, 0] - cx)
    
#     # Push padding/duplicate points to the end
#     sorted_angles = jnp.where(is_unique_edge, angles, 10.0)
#     sorted_idx = jnp.argsort(sorted_angles)
#     ordered_poly = clean_poly[sorted_idx]
    
#     # 4. Collapse padding onto the last valid boundary vertex
#     last_valid_idx = jnp.clip(num_valid - 1, 0, poly.shape[0] - 1)
#     last_valid_val = ordered_poly[last_valid_idx]
...
#     h0 = jnp.array([m_p, -1.0, 0.0])       # y >= m_p * x
    
#     capacity_plane = jnp.array([N, P, κ])
#     dummy_plane = jnp.array([0.0, 0.0, 1.0])
#     h_cap = jnp.where(κ < (P + N), capacity_plane, dummy_plane)
    
#     # Explicitly include the bounding box half-planes matching geometry.py
#     h_x_min = jnp.array([-1.0, 0.0, 0.0])  # x >= 0
#     h_y_max = jnp.array([0.0, 1.0, 1.0])   # y <= 1
#     h_x_max = jnp.array([1.0, 0.0, 1.0])   # x <= 1
#     h_y_min = jnp.array([0.0, -1.0, 0.0])  # y >= 0
...
#     # 2. Check for vertical line slopes (when t -> 1 or x2 -> x1)
#     is_vertical = jnp.abs(dx) < 1e-10
#     safe_dx = jnp.where(is_vertical, jnp.where(dx < 0, -1e-10, 1e-10), dx)
...
#     thresholds: Optional[jnp.ndarray] = None,
#     do_fast_threshold_sel_via_cost: bool = False):
#     """Compute partial VOROS (average of max reduced area across t in range) using JAX.
    
#     Integrates in r-space (cost-ratio space) with uniform sampling, matching 
#     scipy.integrate.trapezoid(max_points, x=fp_cost_ratios) / (max_r - min_r).
#     """
#     # 1. Fetch max reduced areas and ts
#     if return_best_thresholds:
#         max_points, ts, best_thresholds = max_area_per_t(
#             fprs, tprs, κ, α, P, N, min_fp_cost_ratio, max_fp_cost_ratio,
#             n_points=n_points, return_best_thresholds=True, thresholds=thresholds,
...
#     # 2. Build the exact fp_cost_ratios grid in r-space
#     fp_cost_ratios = jnp.linspace(min_fp_cost_ratio, max_fp_cost_ratio, n_points, dtype=jnp.float64)
#     r_range = max_fp_cost_ratio - min_fp_cost_ratio

#     # 3. Trapezoidal integration over x=fp_cost_ratios (scipy.integrate.trapezoid in JAX)
#     # integral = sum( (y_{i+1} + y_i) / 2 * (x_{i+1} - x_i) )
#     dx = (max_fp_cost_ratio - min_fp_cost_ratio) / jnp.maximum(n_points - 1, 1)
    
#     # Vectorized trapezoidal formula for uniformly spaced points
#     integral_val = 0.5 * dx * (max_points[0] + max_points[-1] + 2.0 * jnp.sum(max_points[1:-1]))
    
#     # Normalization: divide integral by r_range
#     vor_integrated = integral_val / jnp.maximum(r_range, 1e-15)

#     # 4. Handle edge cases (n_points <= 1 or r_range <= 0) without dynamic control flow
#     is_valid_range = (n_points > 1) & (r_range > 0)
#     vor = jnp.where(is_valid_range, vor_integrated, max_points[0])
